...
Railway reads `backend/railway.json` and builds the API with
`backend/Dockerfile`. The container command runs `npm run db:push` before
starting the API. The deployment stops if Prisma cannot create or update the
tables, so a broken database setup cannot appear as a healthy release.
